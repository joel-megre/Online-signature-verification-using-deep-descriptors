...
for user in range(1,33,1):
    genuine_indexes = random.sample(population=range(1,21), k=15)
    forged_indexes = random.sample(population=range(21,41), k=15)
...
for user in range (33,41,1):
    evaluation_indexes = list(range(1,41,1))
    for i in evaluation_indexes:
        ...
        with open(write_path_evaluation + file_name + '.txt', 'w') as file:
            file.write('\n'.join(modified_lines))
